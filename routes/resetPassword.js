   ...
   try {
      ...
      if (!decodedEmail) {
         return res.status(400).send({
            statusCode: (400),
            message: 'Oops!, email not found for the specified token, Invalid or expired token.'
         })
      }
      ...
      try{
         const admin = await Admin.findOne({ where: { email:decodedEmail }})
         console.log({admin})
         if(admin){
            admin.email = email,
            admin.password = hashed
            const saveResult = await admin.save()
